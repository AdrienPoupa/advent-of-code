...
const expandedPairs: number[][][] = pairs.map((pairs: string[]) =>
    pairs.map((pair: string) => expandAssignments(pair)),
);

// 2 pairs are fully overlapping if one is fully contained in the other
const arePairsFullyOverlapping = function (
    pair: number[],
    otherPair: number[],
): boolean {
    return (
        pair.every((v) => otherPair.includes(v)) ||
        otherPair.every((v) => pair.includes(v))
    );
};

// Count fully overlapping pairs
const fullyOverlappingPairs: number = expandedPairs.reduce(
    (acc: number, pairs: number[][]) =>
        arePairsFullyOverlapping(pairs[0], pairs[1]) ? acc + 1 : acc,
    0,
);
...
const arePairsOverlapping = function (
    pair: number[],
    otherPair: number[],
): boolean {
    return (
        pair.reduce(
            (acc: boolean, current: number) =>
                acc === false ? otherPair.indexOf(current) !== -1 : true,
            false,
        ) ||
        otherPair.reduce(
            (acc: boolean, current: number) =>
                acc === false ? pair.indexOf(current) !== -1 : true,
            false,
        )
    );
};
...
const overlappingPairs: number = expandedPairs.reduce(
    (acc: number, pairs: number[][]) =>
        arePairsOverlapping(pairs[0], pairs[1]) ? acc + 1 : acc,
    0,
);
