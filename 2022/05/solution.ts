...
const stacksLine: string = parsedInput.find((value: string) => value.indexOf(' 1') !== -1) ?? '';
...
const fillStackContent = function(content: string, stacks: Stack<string>[]): void {
    let currentStack = 0;
...
const fillStacks = function(): Stack<string>[] {
    ...
    const stacksContent: string[] = parsedInput.filter((value: string) => value.indexOf('[') !== -1).reverse()
...
const moveStacks = function(stacks: Stack<string>[], nbMoves: number, fromStackNb: number, toStackNb: number): void {
...
const moves: number[][] = parsedInput.filter((value: string) => value.indexOf('move') !== -1)
    .map((move: string) => move.match(/\d+/g)!.map((move: string) => parseInt(move)))

moves.map((move: number[]) => moveStacks(stacks, move[0], move[1], move[2]))

const peekStacks = function(stacks: Stack<string>[]): string {
    return stacks.reduce((acc: string, value: Stack<string>) => acc + (value.peek() || ''), '')
}
...
const moveMultipleStacks = function(stacks: Stack<string>[], nbMoves: number, fromStackNb: number, toStackNb: number): void {
...
moves.map((move: number[]) => moveMultipleStacks(newStacks, move[0], move[1], move[2]))
