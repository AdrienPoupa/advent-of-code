...
const uniqueCompartments = rucksacks.map(
    (content: string) => [
...
const sharedItems = uniqueCompartments.map((content: Set<string>[]) => [...content[0]].filter(x => content[1].has(x))).flat()
...
const getLetterValue = function (letter: string): number {
    return letter.toLowerCase() === letter ? (letter.charCodeAt(0) - 96) : (letter.charCodeAt(0) - 38)
}

console.log('Sum of the priorities: ' + sumArray(sharedItems.map((item: string) => getLetterValue(item))))
...
console.log('Sum of the priority groups: ' + sumArray(sharedGroupItems.map((item: string) => getLetterValue(item))))
