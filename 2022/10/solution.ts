...
const calculateValues = function (instructions: string[], nbCycles: number): number[] {
    let values: number[] = []
    let x: number = 1
...
const strengths: number[] = calculateValues(instructions, 220)
    ...
    .filter((value: number, i: number) => [20,60,100,140,180,220].indexOf(i) !== -1)

console.log('Total strengths: ' + strengths.reduce((acc: number, item: number) => acc + item))
...
const drawScreen = (): string[][] => {
    const screen: string[][] = new Array(6).fill(0).map(() => new Array(40).fill('.'))
...
const screenOutput: string = drawScreen().reduce((acc: string, bit: string[]) => acc + '\n' + bit.reduce((acc: string, bit: string) => acc + bit),'');
