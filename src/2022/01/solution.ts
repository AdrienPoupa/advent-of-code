...
const individualCalories = input
    .split("\n\n")
    .map((individualCaloriesString) =>
        individualCaloriesString.split("\n").map(Number),
    );
...
const individualSums = individualCalories.reduce((acc, val: number[]) => {
    acc.push(
        val.reduce(
            (partialSum: number, calories: number) => partialSum + calories,
            0,
        ),
    );
    return acc;
...
const sortedSums = individualSums.sort(
    (elf: number, otherElf: number) => otherElf - elf,
);
